...
(function (root, document) {
	...
	var UWP = {
		...
		getNavigation(target) {
			...
			UWP_navigation_request.overrideMimeType('application/xml');
			/* UWP_navigation_request.overrideMimeType('text/html'); */
		...
		createStyles() {
			...
			if (UWP.config.mainColor) {
				...
				[data-layoutType="overlay"] header nav:nth-of-type(1) {
					background-color: ${UWP.config.mainColorDarkened}; // @TODO: Darkened?
				}
				...
				[data-layoutType="docked-minimized"] header nav:nth-of-type(1) {
					background: ${UWP.config.mainColorDarkened}; // @TODO: Darkened?
				}
		...
		navigate(target, addHistory) {
			...
			if (addHistory !== false) {
				history.pushState('', '', `${root.location.href.split('#')[0]}#page=${target}`);
			}
				...
				if (UWP_navigate_request.readyState === 4) {
					if (UWP_navigate_request.status === 200) {
						if (UWP_navigate_request.responseXML) {
							...
							var pageBody = page.querySelector('body').innerHTML;
			...
			UWP_navigate_request.overrideMimeType('application/xml');
			/* UWP_navigate_request.overrideMimeType('text/html'); */
			try {
